...
// shown while the robot is fully asleep (ported from Screen_Emulation5.py).
//
// Screen modes:
...
// ------------------------------------------------------------
// Face registry -- 1:1 port of eye_engine.py (SHAPES_REGISTRY +
// COLOR_PALETTE). Each shape draws BOTH eyes, same signature as the
// Python: (lx, rx, my, w, h, color). Add entries to the tables at the
// bottom -- the menu picks up the new counts automatically.
// ------------------------------------------------------------
typedef void (*EyeDrawFn)(int lx, int rx, int my, int w, int h, uint16_t col);

struct Pt { int x, y; };

// Scanline polygon fill, even-odd rule (same as pygame.draw.polygon),
// so non-convex / self-crossing outlines like Hourglass and Chevron
// come out identical to the emulator.
...
// Top or bottom half of the ellipse inscribed in (x, y, w, h).
// NOTE: matches the emulator exactly -- in eye_engine.py's
// draw_semi_circle, blit(..., area) pastes the clipped half at
// rect.topleft, so the BOTTOM half is drawn in the UPPER half of the box
// (a bowl sitting above centre). Set drawAt to y + h/2 instead of y if you
// ever want it mathematically centred below the midline.
...
// Python's // on negatives floors; all divisions here are of positive
// values, so C integer division matches.
...
// ------------------------------------------------------------
// Settings persistence (own NVS namespace + own Preferences object,
// so it never collides with the gait task's servo/IMU saves)
// ------------------------------------------------------------
...
// ------------------------------------------------------------
// Eye animator -- shared by the standing face and the Face Settings
// preview. Smooth (multi-frame) blink as in the pygame mock. Only
// redraws when something visible changed, so a static face never flickers.
// ------------------------------------------------------------
struct EyeAnimState {
...
// Main-page row numbers. Emote Mode is first, so everything else shifted
// down by one -- named rather than bare indices so they stay in step.
// ROW_BODY is the "Active Gait Settings" page: COG offset, stand height and
// step height. All three affect ACTIVE WALKING only -- emote mode and puppet
// mode each drive their own body height, and step height is a gait-only term.
...
void menuImuZero() {
    // The IMU now updates in the GAIT task, not this one, so this function
    // must NOT touch the filter state directly -- it would be writing
    // body_*_filtered and the Kalman covariance while the gait task is
    // mid-update at 120 Hz. Request it instead; the gait task applies the
    // zero between its own samples and hands back a completion flag.
...
// Only A ever opens a page or saves; L1/R1/L2/R2 never do.
void menuHandleEvent(uint8_t ev) {
    ...
    case LAYER_MAIN:
        if (down)      { menuIdx = (menuIdx + 1) % MENU_MAIN_COUNT; menuNeedsPageRedraw = true; }
            ...
            switch (menuIdx) {
                case ROW_EMOTE:
                    // Ask the gait task to stand up normally; it switches
                    // emote mode on once fully standing, and the emote list
                    // then takes over the screen.
    ...
    case LAYER_IMU:
        if (down)      { menuIdx = (menuIdx + 1) % MENU_IMU_COUNT; menuNeedsPageRedraw = true; }
            ...
            if (menuIdx == IMU_ROW_ZERO) {
                ...
                // Ask the gait task to stand up, then run the bow/tilt dance.
                // It stays standing afterwards, so the robot lands in active
                // mode ready to walk.
    ...
    case LAYER_EMOTE:
        // Standing, emote mode on. L1/R1 pick, A plays, B leaves emote mode
        // (staying standing). Mode stays on after an emote ends, so the list
        // is still up to pick another.
        if (NUM_EMOTES == 0) {
...
// ------------------------------------------------------------
// Menu rendering. GLCD font size 2 = 12x16 px per char; every line is
// padded to a fixed width and drawn with a background colour, so text
// overwrites itself cleanly without clearing (no flicker).
// ------------------------------------------------------------
...
void menuDrawPage() {
    ...
    case LAYER_EMOTE: {
        // While an emote plays, the list gets out of the way and the face
        // comes up -- then the list is fully redrawn when playback ends.
        ...
        // Keep the selected row on screen without redrawing the whole list
        // every tick (the playback timer below updates ~10Hz).
...
void menuRender() {
    // Kill switch (or anything else) dropped us out of calibration mode
    // while on the servo page -> treat as cancel.
...
void TelemetryTask(void * pvParameters) {
    ...
    for(;;) {
        // IMU sampling moved to the gait task (fixed 120 Hz). This task now
        // only READS the published attitude values, so screen work can no
        // longer affect the balance loop's sample rate.
        ...
        // Persist a level zero once the gait task has applied it (NVS
        // writes are far too slow to do inside the 120 Hz gait loop).
        ...
        // ----- Which screen? -----
        ScreenMode mode;
        // The bow/tilt dance moves the robot on its own. It gets its own
        // screen so it is never mistaken for the robot misbehaving, and so
        // the "do not touch" instruction is actually visible -- the manual
        // path deliberately skips the footing-stable gate.
        ...
        if (mode == SCREEN_MENU) {
            ...
            // Sleep until the next button press OR the refresh period --
            // input feels instant even though the idle refresh rate is low.
        ...
        if (mode == SCREEN_EYES) {
            ...
            // Countdown before the robot puts itself to sleep, so it is
            // never a surprise. Only redrawn when the number changes.
