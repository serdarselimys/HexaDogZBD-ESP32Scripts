...
// Seconds for compensation to fade 0->full once balance mode is toggled.
...
// Byte   21   : selected_emote_id -- UNCHANGED
// ---- everything past here is optional; a bare 22-byte packet still
//      drives the robot exactly like before. ----
// Byte   22   : buttons2 -- 0x01 manual re-cal trigger (edge)
//                           0x02 KILL SWITCH (level-triggered, checked first, every tick)
//                           0x04 RESERVED (was integral term enabled)
//                           0x08 tunables-valid (if unset, bytes 24-47 are ignored --
//                                so the controller only overrides values the user
//                                actually touched, instead of stomping auto-cal's
//                                derived axis mapping every packet)
//                           0x10 RESERVED (was legacy-compensation A/B toggle)
//                           0x20 RESERVED (was ramp assist enabled)
//                           0x40 L2 pressed (level) -- manual pitch trim, nose down (walking mode only)
//                           0x80 R2 pressed (level) -- manual pitch trim, nose up (walking mode only)
// Byte   23   : reserved / padding
// Bytes 24-27 : LEVEL_GAIN                (float32, only if buttons2 & 0x08)
// Bytes 28-31 : KALMAN_Q_ANGLE             (float32, only if buttons2 & 0x08)
// Bytes 32-35 : KALMAN_Q_BIAS              (float32, only if buttons2 & 0x08)
// Bytes 36-39 : KALMAN_R_MEASURE           (float32, only if buttons2 & 0x08)
// Bytes 40-43 : LEVEL_DEADBAND_DEG         (float32, only if buttons2 & 0x08)
// Bytes 44-47 : LEVEL_MAX_TILT_DEG         (float32, only if buttons2 & 0x08)
// ============================================================
...
// ============================================================
// A mode in its own right, like body leveling: while it is on, body
// height is forced to PUPPET_BODY_HEIGHT and leveling, idle motion,
// height trim and manual pitch trim are all disabled. Nothing else drives
// the body, so nothing fights over the same axes.
//
// THE MATH: commanded foot positions are in the BODY frame. With the feet
// planted, commanding f_body = R^T . f_nominal leaves exactly one body
// attitude consistent with all six feet on the same plane: R. The body is
// forced into that pose. Note the TRANSPOSE -- real leveling on a slope
// needs the forward R. Mirroring needs R^T.
//
// PHONE HELD IN PORTRAIT: top of the screen is the robot's nose.
...
// The screen task owns the menu; the gait task owns standing and emote
// playback. These three flags are the only things crossing between them.
//   emote_menu_requested : set by the screen when "Emote Mode" is chosen
//                          while asleep. The gait task stands the robot up
//                          normally, then turns emote mode on -- rather than
//                          letting the emote height ramp do an unsupervised
//                          stand-up that skips the footing/settle checks.
//   emote_play_request   : set by the screen when A is pressed on a list row
//   emote_exit_request   : set by the screen when B leaves the emote list
